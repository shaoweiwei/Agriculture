...
//			}
	...
	public static <T> String setList(String key ,List<T> list){
		...
		try {
			...
			result = jedis.set(key.getBytes(),ObjectUtils.serialize(list));
			jedis.expire(key, EXPIRE_TIME);//设置key的生存时间
			logger.debug("setList {} = {}", key, list.size());
		} catch (Exception e) {
			logger.warn("setList {} = {}", key, list.size(), e);
		} finally {
			returnResource(jedis);
		}
		return result;
	 ...
	 * @return list
	 */
	public static <T> List<T> getListT(String key){
		List<T> list = null;
		Jedis jedis = null;
		try {
			jedis = getResource();
			if (jedis.exists(key)) {
				byte[] invalue = jedis.get(key.getBytes());
				list = (List<T>) ObjectUtils.unserialize(invalue);  
				logger.debug("getList {} = {}", key, invalue);
			}
		} catch (Exception e) {
