...
public class CustomSessionManager {
	 ...
	 * session status 
	 */
	public static final String SESSION_STATUS ="dty-online-status";
	@Autowired
	 ...
	 * @return
	 */
	public  List<Principal> getAllUser() {
		//获取所有session
		Collection<Session> sessions = sessionDAO.getActiveSessions();
		List<Principal> list = new ArrayList<Principal>();
		
		for (Session session : sessions) {
			Principal bo = getSessionBo(session);
			if(null != bo){
				list.add(bo);
			}
		}
	...
	private Principal getSessionBo(Session session){
		//获取session登录信息。
		Object obj = session.getAttribute(DefaultSubjectContext.PRINCIPALS_SESSION_KEY);
		if(null == obj){
			return null;
		...
		if(obj instanceof SimplePrincipalCollection){
			...
			if(null != obj && obj instanceof User){
				//存储session + user 综合信息
				Principal userBo = new Principal((User)obj,false);
				//最后一次和系统交互的时间
				...
				//session最后一次与系统交互的时间
				//userBo.setLoginDate(session.getLastAccessTime());
				//回话到期 ttl(ms)
	...
	public void setSessionDAO(JedisSessionDAO sessionDAO) {
